...
async function main() {
  ...
  record(
    ...
    !rpcErr && !!inquiryId,
    rpcErr?.message ?? String(inquiryId),
  );
  ...
  record(
    "anon direct SELECT on inquiries blocked",
    anonSelBlocked,
    anonSelErr?.message ?? `rows=${(anonInq ?? []).length}`,
  );
  ...
  record(
    "normal user cannot read other users' inquiries",
    otherInq.length === 0,
    `otherRows=${otherInq.length}`,
  );

  const admin = await signIn(ADMIN_EMAIL);
  const { data: adminInq, error: adminInqErr } = await admin
    .from("inquiries")
    .select("id")
    .limit(5);
  record(
    "admin can read inquiries",
    !adminInqErr && (adminInq ?? []).length > 0,
    adminInqErr?.message ?? `rows=${(adminInq ?? []).length}`,
  );
  ...
  if (inquiryForNote) {
    ...
    const { data: profiles } = await admin
      ...
      .maybeSingle();
    record(
      "author profile resolvable for inquiry notes",
      !!profiles?.email,
      profiles?.email ?? "missing",
    );
  } else {
    record("admin inquiry note tests", null, "no inquiry row");
  }

  const { data: normalNotes, error: normalNotesErr } = await normal
    .from("inquiry_notes")
    .select("id")
    .limit(1);
  record(
    "inquiry notes admin-only (normal blocked)",
    !!normalNotesErr || (normalNotes ?? []).length === 0,
    normalNotesErr?.message,
  );
  ...
  const { data: normalRegs } = await normal
  ...
  const onlyOwnRows =
    !normalAttErr &&
    (normalAtt ?? []).every(
      (row) => !row.walk_in_id && (!row.registration_id || ownRegIds.has(row.registration_id)),
    );
  record(
    "normal user sees only own attendance rows (not walk-ins/admin-wide)",
    onlyOwnRows,
    normalAttErr?.message ?? `rows=${(normalAtt ?? []).length}`,
  );

  const { data: normalWalkIns, error: normalWalkErr } = await normal
    .from("event_walk_ins")
    .select("id")
    .eq("event_id", DEMO_EVENT)
    .limit(5);
  record(
    "normal user cannot read walk-in records",
    !!normalWalkErr || (normalWalkIns ?? []).length === 0,
    normalWalkErr?.message,
  );
  ...
  const { data: adminAtt, error: adminAttErr } = await admin
    ...
    .limit(5);
  record(
    "admin can read attendance",
    !adminAttErr && (adminAtt ?? []).length > 0,
    adminAttErr?.message ?? `rows=${(adminAtt ?? []).length}`,
  );

  // --- Evaluations ---
  const { data: normalEval, error: normalEvalErr } = await normal
  ...
  record(
    "normal user sees only own evaluation responses",
    onlyOwnEval,
    normalEvalErr?.message ?? `rows=${(normalEval ?? []).length}`,
  );

  // --- Demographics ---
  const { data: normalDemo, error: normalDemoErr } = await normal
    .from("participant_demographics")
    .select("id")
    .limit(1);
  record(
    "raw demographic rows protected from normal users",
    !!normalDemoErr || (normalDemo ?? []).length === 0,
    normalDemoErr?.message,
  );

  const { data: agg, error: aggErr } = await admin.rpc("get_event_demographic_aggregates", {
    p_event_id: DEMO_EVENT,
  });
  const suppressed =
    Array.isArray(agg) &&
    agg.some(
      (r) =>
        String(r.bucket_label ?? r.label ?? "").includes("Fewer than 5") || r.suppressed === true,
    );
  record(
    "demographic aggregate RPC returns suppressed small cells",
    !aggErr && (suppressed || (agg ?? []).length >= 0),
    aggErr?.message ?? `rows=${(agg ?? []).length}`,
  );
  ...
  const allApproved =
    !pubGalErr && (publicGallery ?? []).every((g) => g.is_public_approved === true);
  record(
    "public gallery query returns only approved rows",
    allApproved,
    pubGalErr?.message ?? `approved=${(publicGallery ?? []).length}`,
  );
  ...
  const { data: pendingGallery } = await admin
    ...
    .limit(5);
  record(
    "unapproved submissions exist separately from public set",
    true,
    `pending=${(pendingGallery ?? []).length}`,
  );

  // --- Reports ---
  const { data: normalReports, error: normalReportsErr } = await normal
    .from("event_report_snapshots")
    .select("id")
    .limit(1);
  record(
    "normal user cannot access report snapshots",
    !!normalReportsErr || (normalReports ?? []).length === 0,
    normalReportsErr?.message,
  );

  const { data: adminReports, error: adminReportsErr } = await admin
    .from("event_report_snapshots")
    .select("id, status")
    .limit(5);
  record(
    "admin can read report snapshots",
    !adminReportsErr,
    adminReportsErr?.message ?? `rows=${(adminReports ?? []).length}`,
  );

  // --- Profiles ---
  const { data: ownProfile, error: ownProfileErr } = await normal
    .from("profiles")
    .select("id, email")
    .eq("id", (await normal.auth.getUser()).data.user.id)
    .maybeSingle();
