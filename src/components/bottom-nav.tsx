...
export function BottomNav() {
  ...
  return (
    <nav className="fixed inset-x-0 bottom-0 z-40 border-t border-border/60 bg-card/95 backdrop-blur-lg md:hidden safe-bottom">
        ...
        {items.map((it) => {
          ...
          return (
            <Link
              key={it.to}
              to={it.to}
              className="group flex flex-1 flex-col items-center gap-0.5 rounded-xl px-2 py-1.5 text-[10px] font-medium transition"
            >
